...
const (
	...
	// 应用的API Key
	apiKey string = "xvOhGOL59fstIzcVh57yUwXm"
	// 应用的Secret Key
	secretKey string = "G3E26ZbnN3BkbQqFLagrQPNbtcdbzp7x"
	// 授权服务地址
...
func Recongntion(imgPath string)(result,account string){
	var results=AccurateBasic(imgPath).WordsResult
	account=results[3].Words[8:]
	for _,i := range results {
		if i.Words[:2]=="ha" {
			result = i.Words[5:]
			fmt.Println("a",i.Words[5:])
		}
	}
